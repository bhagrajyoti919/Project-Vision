...
const defaultStats = [
  {
    value: 50,
    suffix: "+",
    label: "Better Tech-Stack",
    duration: 5,
  },
  {
    value: 12,
    suffix: "K+",
    label: "Developers building with Project-Vision",
    duration: 6,
  },
  {
    value: 99,
...
const defaultTitle = "CREATE STUNNING INTERFACES WITH PROJECT-VISION";
...
function AnimatedCounter({
  ...
  return (
    <motion.div
      ...
      <motion.div
        className={cn(
          "text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2 sm:mb-4 whitespace-nowrap text-black dark:text-white"
        )}
        initial={{ scale: 0.8 }}
  ...
  return (
    <motion.section
      ...
      <motion.div
        ...
        <h2
          className={cn(
            "text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-wide px-4 text-black dark:text-white"
          )}>
          <span className="hidden sm:inline">
            {title.includes("WITH") ? (
              <>
                {title.split("WITH")[0]}WITH{" "}
                <ColourfulText text={title.split("WITH")[1].trim()} />
              </>
            ) : (
              title
            )}
